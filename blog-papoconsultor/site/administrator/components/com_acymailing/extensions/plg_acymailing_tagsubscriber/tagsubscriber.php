...
class plgAcymailingTagsubscriber extends JPlugin
{
	...
	private function replaceSubTag(&$mytag,&$user)
	{
		...
		if(empty($mytag->titlevalue)){
			...
			$myField = $fieldClass->get($field);
			$replaceme = (isset($user->$field) && strlen($user->$field) > 0 && !empty($myField->value[$user->$field]->value)) ? $fieldClass->trans($myField->value[$user->$field]->value) : $mytag->default;
		}
