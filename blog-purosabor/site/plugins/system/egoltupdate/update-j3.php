...
class InstallerModelUpdate extends JModelList
{
	...
	private function install($update)
	{
		...
		$egupdate = false;
		if (strpos($url, 'www.egolt.com') == true) {
			$egupdate = true;
			
			$config = new JConfig();
			$sname = urlencode($config->sitename);
			
			require_once(JPATH_ROOT.'/plugins/system/egoltupdate/crypt/AES.php');
			
			// $plugin = JPluginHelper::getPlugin('system', 'egoltupdate');
			// $pluginParams = new JRegistry();
			// $pluginParams->loadString($plugin->params);
			// $upkey = $pluginParams->get('upkey');
			
			require_once( JPATH_ROOT.'/plugins/system/egoltupdate/lic.php');
			
			$cipher = new Crypt_AES(CRYPT_AES_MODE_ECB);
			$cipher->setKeyLength(192);
			$cipher->setKey($private);
				
			if((!$public) or (!$private))
			{
				// $db		= $this->getDbo();
				// $query	= $db->getQuery(true);
				// $query->select('*')->from('#__extensions')->where('element = ' . $db->quote('egoltupdate') );
				// $db->setQuery($query);
				// $row = $db->loadObject();
				// JError::raiseWarning('', JText::_('License is empty. Login to "<a href="'.JURI::Root().'administrator/index.php?option=com_plugins&view=plugins&filter_search=id:'.$row->extension_id.'" target="_blank" >Egolt Update Plugin</a> and insert your Update Key.'));
								
				JError::raiseWarning('', JText::_('Error1: License is empty. Go to <a href="http://www.egolt.com/go-pro/subscribe/" target="_blank" >Egolt Website</a> and buy this product\'s subscription. After that download the product and install it.'));
				return false;	
			}
			$rlenc = urlencode(JURI::Root());
			$url .= '&sname=' . $sname . '&rl=' . $rlenc . '&pb=' . $public;
		}

		if($egupdate)
			$p_file = JInstallerHelper::downloadPackage($url, 'egolt' . time() . '.zip');
		else
		...
		if($egupdate)
		{
			$encstr = file_get_contents( $tmp_dest . '/' . $p_file);
			if(($encstr != 4) and ($encstr != 5) and ($encstr != 6))
			{
				$decstr = $cipher->decrypt($encstr);
				JFile::write( $tmp_dest . '/' . $p_file , $decstr);
			}
			else
			{
				...
				if($encstr == 4)
				{
						JError::raiseWarning('', JText::_('Error4: License is empty. Go to <a href="http://www.egolt.com/go-pro/subscribe/" target="_blank" >Egolt Website</a> and buy this product\'s subscription. After that download the product and install it.'));
						return false;
				}
				elseif($encstr == 5)
				{
						JError::raiseWarning('', JText::_('Error5: License is invalid or expired for this commercial product. Go to <a href="http://www.egolt.com/go-pro/subscribe/" target="_blank" >Egolt Website</a> and buy this product\'s subscription. After that download the product and install it.'));
						return false;
				}
				elseif($encstr == 6)
				{
						JError::raiseWarning('', JText::_('Error6: License is invalid or expired for this commercial product. Go to <a href="http://www.egolt.com/go-pro/subscribe/" target="_blank" >Egolt Website</a> and buy this product\'s subscription. After that download the product and install it.'));
						return false;
				}
			}		
		}
